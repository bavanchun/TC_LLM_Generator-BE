...
public class ProjectMemberController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<ProjectMemberResponse>> getProjectMemberById(@PathVariable("id") UUID id) {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<PagedModel<ProjectMemberResponse>>> getAllProjectMembers(
            @PageableDefault(size = 20, sort = "joinedAt", direction = Sort.Direction.DESC) Pageable pageable) {

        Page<ProjectMember> page = projectMemberService.getAllProjectMembers(pageable);
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse<Void>> removeProjectMember(@PathVariable("id") UUID id) {
        if (!projectMemberService.projectMemberExists(id)) {
            throw new ResourceNotFoundException("ProjectMember", "id", id);
        }
